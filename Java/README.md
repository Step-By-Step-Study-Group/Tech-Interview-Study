...
### 디스패처 서블릿의 동작 방식
![img](https://user-images.githubusercontent.com/97332044/231753288-0e139096-0e22-44db-bb6a-950a14da066e.jpg)

**디스패처 서블릿을 통해 요청을 처리할 컨트롤러를 찾아서 위임하고 그 결과를 받아옴**

- 클라이언트로부터 HTTP 요청을 받음
- 요청 URL에 따라 적절한 핸들러를 찾기 위해 URL 매핑을 수행
- 찾아낸 핸들러가 적절한지 검증
- 해당 핸들러를 실행
- 핸들러의 실행 결과를 HTTP 응답으로 변환
- 클라이언트에게 HTTP 응답을 반환
...
1. 단일 타입만 사용하는 경우
    ...
    - 타입의 유연성을 보장해주는 제네릭은 컴파일 단계에서 타입 체크를 수행한다. 
    - 이 때문에 여러가지 타입을 사용할 수 있지만 사용하는 타입이 하나의 타입만 다루는 경우는 해당 타입을 지정해주는것이 코드가 간결해지고 가독성이 좋아진다
    
2. 타입 관련 오류를 발생시키지 않는 경우

   - 제네릭을 사용하지 않으면 컴파일러가 타입 관련 오류를 검사하지 않음 → 이를 방지하기 위한 코드를 작성해야함
   - 이 때문에 코드의 길이가 늘어나고 복잡도가 높아질 수 있다

3. 코드의 복잡도가 높아지는 경우

   - 코드의 길이와 복잡도가 높아질 수 있다
   - ex) 타입 파라미터, 와일드카드 등의 새로운 문법을 사용해야 하므로 이해하는데 시간이 걸릴 수 있다
...
- 매개변수가 많은 생성자를 대체할 수 있음 → 코드의 가독성을 높힘
- 객체를 생성할 때 어떤 값이 어떤 필드에 설정되는지 알기 쉽다
- 불필요한 `null` 값이 들어가지 않도록 한다
- 객체 생성 시에 필드에 대한 유효성 검사를 수행 → 필드의 값이 `null`인 경우 예외를 발생
- 생성자를 추가로 작성하지 않아도 되므로 코드 양을 줄일 수 있다
...
### 변수
- 값을 변경할 수 없는 변수로 정의
- 값을 한 번 할당하면 변경할 수 없음
- 즉시 초기화되거나 생성자에서 초기화됨

### 메서드 
- 하위 클래스에서 메서드를 재정의 할 수 없음

### 클래스
- 하위 클래스를 가질 수 없는 클래스로 정의됨

## 장점

### 안전성

- `final` 변수를 사용하면 값이 **변경되는 것을 방지**할 수 있음
- 버그를 방지하고 안정성을 높일 수 있음

### 보안

- `final` 클래스를 사용하면 하위 클래스를 생성하지 못하도록 할 수 있음
- 클래스의 **구현을 보호**하고 상속에 대한 위험을 줄일 수 있음

## 단점

### 유연성 감소
- `final` 변수는 값이 변경되지 않기 때문에 **유연성이 감소**함
- `final` 변수를 사용하면 값을 변경해야 하는 경우 추가적인 코드 수정이 필요

### 가독성 저하
- 코드를 **복잡도 증가**

### 상속 제한
- `final` 클래스를 사용하면 하위 클래스를 생성할 수 없음
- 상속에 대한 유연성이 제한적
...
- 하위 클래스의 객체를 상위 클래스로 변환
- 자식 클래스가 부모 클래스를 상속하는 경우 → 자식 클래스 객체는 부모 클래스 객체로 자동 형 변환
- 부모 클래스의 메서드만 사용 가능 → 다형성을 활용하는데 유용

## 다운캐스팅
- 상위 클래스의 객체를 하위 클래스로 변환
- 업캐스팅과는 달리 명시적으로 타입을 지정해야 함
- 다운캐스팅은 업캐스팅과 달리 자식 클래스의 메서드도 사용 가능 → 구체적인 작업을 처리할 때 유용

# Wrapper Class

`Wrapper Class` 는 기본타입을 객체화시키는 클래스
...
- `Wrapper Class`는 기본 데이터 타입과 달리 `null`값을 허용하기 때문에 `null`값을 저장하거나 반환할 수 있음
- 자바의 컬렉션 프레임워크는 객체만을 다루기 때문에 컬렉션을 사용하기 위해서는 `Wrapper Class`를 사용해야 함
- 기본 데이터 타입을 사용하는 경우에는 값을 비교하기 위해서 == 연산자를 사용하지만 `Wrapper Class` 를 사용하는 경우에는 `equals()` 메소드를 사용하여 값을 비교할 수 있음
 ...
 **자료구조**를 표현하는 **클래스**와 **인터페이스**의 집합
 
 ## 특징
 
 - **재사용성** : 자료구조를 구현하는 코드를 직접 작성할 필요가 없음 → 재사용성이 높아짐

- **유연성** : 다양한 자료구조를 지원 → 데이터를 저장하고 검색하는 데 필요한 자료구조를 선택할 수 있음

- **편의성** : 다양한 메서드를 제공 → 자료구조를 쉽게 활용할 수 있음

- **안전성** : 제네릭을 사용하여 **컴파일 시점**에서 타입 안전성을 보장

## 단점

- 컬렉션 프레임워크를 사용하면 불필요한 **메모리 사용량이 증가**할 수 있음

- 대용량 데이터 처리나 빠른 검색이 필요한 경우에는 **성능에 영향**을 미칠 수 있음

- 인터페이스와 클래스를 잘 이해해야 하며 다양한 메소드의 **러닝커브**가 있을 수 있음
