...
[Transaction이란?](#transaction이란)<br/>
[Transaction의 교착상태란?](#transaction의-교착상태란)<br/>
...
## 교착상태(Deadlock)
- 둘 이상의 트랜잭션이 하나의 자원을 동시에 요청하고 점유하고 있을 때, 서로가 상대방의 자원 점유를 기다리면서 무한정 대기 상태에 빠지는 문제
- 각 트랜잭션은 다른 트랜잭션이 점유하고 있는 자원을 요청하며 서로 대기 상태에 빠지기 때문에 트랜잭션들이 모두 종료되지 않아 데이터베이스 시스템이 멈추는 문제가 발생할 수 있음

## 해결방법 

### 교착상태 예방
- 교착상태가 발생하지 않도록 **트랜잭션 수행 순서를 미리 결정**하여 교착상태를 예방하는 방법
- 모든 경우에 대해 미리 예측하기 어려우며 시스템 성능에 영향을 끼칠 수 있음

### 교착상태 탐지 및 회피
- **시스템이 교착상태를 탐지**하고 교착상태에 빠지지 않도록 자원을 할당하는 방법
- 트랜잭션의 대기 상태를 감시하고 교착상태가 발생할 가능성이 있는 경우 자원을 반납하거나 선점하여 다른 트랜잭션이 사용할 수 있도록 함

### 교착상태 해제
- 이미 교착상태에 빠진 트랜잭션들을 해제하는 방법
- 대기 중인 트랜잭션 중 하나를 선정하여 **해당 트랜잭션의 자원 점유를 해제**하고 다른 트랜잭션이 해당 자원을 이용할 수 있도록 하며 이 과정을 반복하여 교착상태를 해제할 수 있음
