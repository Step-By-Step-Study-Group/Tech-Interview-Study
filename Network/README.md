...
[HTTP메소드 GET과 POST의 차이](#http메소드-get과-post의-차이)<br/>
[HTTP메소드 PUT과 PATCH의 차이](#http메소드-put과-patch의-차이)
...
- 요청 본문에 포함된 데이터로 리소스 **전체를 대체**
- 데이터를 HTTP요청 메시지 `Body`에 포함시켜 전송
- `Body` 에는 클라이언트가 전체 리소스를 알고 있고 업데이트하려는 **데이터를 모두 포함**하고 있을 때 사용
- 폼 데이터나 JSON 등 다양한 형태로 전송 가능
...
- PUT 메소드와 유사
- 요청 `Body` 에는 **수정하려는 필드만** 포함<br/>
- 클라이언트가 **리소스의 일부**만 알고 있거나 업데이트할 **데이터가 일부분**일 때 사용

## 차이

### 사용목적

> `PUT` 은 **전체 리소스를 업데이트**이며 클라이언트가 리소스의 전체를 알고 있어야 함<br/>
`PATCH` 는 **일부 리소스를 업데이트** 클라이언트가 일부분만 알고 있어도 됨
> 

### 멱등성

> `PUT` 은 **멱등성을 만족**하여 여러 번 실행해도 결과가 동일<br/>
`PATCH` 는 **멱등성을 만족하지 않을 수 있음**
