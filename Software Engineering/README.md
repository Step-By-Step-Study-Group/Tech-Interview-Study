...
[MSA와 기존 아키텍처와의 차이점](#msa와-기존-아키텍처와의-차이점)<br/>
...
### 함수의 조합을 통한 모듈화
- 함수형 프로그래밍에서 함수는 **일급 객체**로 취급
- **일급 객체** 이용해 함수를 간단한 단위로 쪼개고, 조합하거나 합성함으로써 문제를 해결
- 
### 상태를 불변으로 처리
- 상태를 불변으로 처리
- 함수의 부작용을 최소화
...
### 함수의 순수성
- 함수의 부작용을 최소화하기 위해 **순수 함수**를 사용
- **순수 함수**는 동일한 입력에 대해 항상 동일한 출력을 반환하며, 외부 상태에 영향을 주지 않는 함수

### 높은 추상화 수준
- 높은 추상화 수준을 가짐
- 함수형 프로그래밍이 수학적 기반을 갖고 있기 때문에 코드를 간결하게 유지하고, 재사용성을 높일 수 있음
<br/>
<details>
<summary>장점</summary>
<div markdown="1">
...
- 초기 학습 비용이 높음
- 성능 면에서는 절차적 프로그래밍과 비교해 느릴 수 있음
- 일부 상황에서는 코드 가독성이 낮아질 수 있음
